...
-- Interrogazioni
select ord_cod from ordini where neg_cod = '0040' union select ord_cod from ordart where ordart_qta > 10;
select distinct com_cod from compart where compart_qta < (select avg(compart_qta) from compart);
select * from articoli where art_cod not in (select art_cod from ordart);
select com_costo - (select avg(com_costo) from componenti) from componenti where com_descrizione = 'Anta 100 cm';